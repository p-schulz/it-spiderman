...
public partial class PlayerMachine : SuperStateMachine
{
    ...
    void Start() {
        ...
        currentState = PlayerStates.EnterLevel;
    }
    ...
    public bool Airborn() {
        if (StateCompare(PlayerStates.Jump) ||
            ...
            StateCompare(PlayerStates.Dive) ||
    ...
    public void MegaSpring(Vector3 direction, float velocity, float lift) {
        ...
        currentState = PlayerStates.MegaSpring;
        return;
    }
    ...
    /// <summary>
    /// Checks if Player is currently standing on level ground. Refer to the SuperCharacterController documentation for a more
    ...
    private bool FootStrike(out GameObject struckObject) {
        ...
        foreach (var col in colliders)
        {
            var machine = col.gameObject.GetComponent<VillainMachine>();
    ...
    void OnGUI()
    {
        if (DebugGui)
        {
            GUI.Box(new Rect(10, 10, 200, 180), "Player Cont");
